    ...
    <section style="padding:40px 0 60px;">
        <div class="container" style="max-width:800px;">
                        ...
                        <div class="ri-media-strip">
                            @foreach ($photos as $i => $photo)
                                <button type="button"
                                    ...
                                    <img src="{{ $photo }}"
                                         alt="{{ $rechercheInnovation->titre }} — photo {{ $i + 1 }}"
                                         loading="lazy">
                                </button>
                            @endforeach
                        </div>
            ...
            @if ($rechercheInnovation->chapo)
                <p style="font-size:18px; font-weight:600; color:var(--forest-deep); overflow-wrap:anywhere; word-break:break-word;">{{ $rechercheInnovation->chapo }}</p>
            @endif
            ...
            {{-- Vidéo : iframe si YouTube / Vimeo / Dailymotion, sinon lien externe --}}
            @if ($rechercheInnovation->embed_url)
                <div style="aspect-ratio:16/9; margin-top:32px; background:#0b140f; border:1px solid var(--line);">
                    <iframe
                        src="{{ $rechercheInnovation->embed_url }}"
                        title="{{ $rechercheInnovation->titre }}"
